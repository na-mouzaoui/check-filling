...
export function exportHistoryToExcel(checks: Check[], users: User[]) {
  const wb = XLSX.utils.book_new()
  ...
  const checksData = [
    ...
    ...sortedChecks.map((check, index) => {
      ...
      return [
        ...
        emissionDate.toLocaleDateString("fr-FR"),
        userMap[check.userId] || "Inconnu",
        check.bank,
        check.payee,
        check.city,
  ...
  XLSX.writeFile(wb, `historique_cheques_complet_${new Date().toISOString().split("T")[0]}.xlsx`)
}
